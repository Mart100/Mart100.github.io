...
function isInsideWalls(p) {
  let PaP = p.position
  let output = true

  if(PaP.x < 1) output = false
  if(PaP.y < 1) output = false
  if(PaP.z < 1) output = false
  if(PaP.x > canvas.width) output = false
  if(PaP.y > canvas.height) output = false
  if(PaP.z > 1000) output = false
