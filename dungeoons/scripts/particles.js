class Particle {
	...
	tick() {
		...
		if(this.velocity) {
			...
			let movement = this.velocity.clone().divide(100)

			// collision
			let nextTileX = world.grid.getTile(Math.round(this.pos.x+movement.x), Math.round(this.pos.y))
			if(!nextTileX) nextTileX = {ground: 'empty'}
			let nextTileY = world.grid.getTile(Math.round(this.pos.x), Math.round(this.pos.y+movement.y))
			if(!nextTileX) nextTileY = {ground: 'empty'}

			if(movement.x > 0 && nextTileX.ground == 'empty') this.velocity.multiply(new Vector(-1, 1))
			if(movement.x < 0 && nextTileX.ground == 'empty') this.velocity.multiply(new Vector(-1, 1))
			if(movement.y > 0 && nextTileY.ground == 'empty') this.velocity.multiply(new Vector(1, -1))
			if(movement.y < 0 && nextTileY.ground == 'empty') this.velocity.multiply(new Vector(1, -1))
			if(nextTileX.ground == 'empty' || nextTileY.ground == 'empty') movement.multiply(-1)
			...
			// gravity
			if(this.gravity) {
				this.velocity.plus(new Vector(0, 0.01))
			}
